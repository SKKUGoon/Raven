...
| File | Table | Used by |
|------|--------|--------|
| `create_table_bar__time.sql` | Time bar table (e.g. `mart.bar__time`) | bar_persistence |
...
- **Schema**: default schema name is `mart` (configurable via timescale config in application). The mart schema contains derived/aggregated data from raw sources stored in InfluxDB.
