...
The Rust crate lives in `src/`, split by responsibility: `server/` for request routing, `data_engine/` for storage, `data_handlers/` for market feeds, `monitoring/` for observability helpers, and `subscription_manager/` for client state. Shared types belong in `src/lib.rs` or module `mod.rs` files. Generated gRPC contracts sit in `proto/`; rebuild bindings with `cargo build` after edits. Configuration defaults are layered in `config/`, with release bundles under `publish/`. Automated tests live in `tests/`, with unit suites organized under `tests/unit/` and integration flows grouped in `tests/integration/`. The `python_client/` folder supplies a smoke-test client aligned with the protobuf schema.
