...
use crate::server::exchanges::websocket::WebSocketParser;
...
impl WebSocketParser for BinanceSpotParser {
    ...
    fn parse_ticker(&self, message: &str) -> RavenResult<Option<MarketDataMessage>> {
        let value: Value = serde_json::from_str(message)
            .map_err(|e| crate::raven_error!(parse_error, format!("Failed to parse JSON: {e}")))?;

        if let Some(stream) = value.get("stream").and_then(|s| s.as_str()) {
            if stream.ends_with("@ticker") {
                let data = value.get("data").ok_or_else(|| {
                    crate::raven_error!(parse_error, "Missing data field".to_string())
                })?;

                let symbol = data.get("s").and_then(|s| s.as_str()).ok_or_else(|| {
                    crate::raven_error!(parse_error, "Missing symbol".to_string())
                })?;
                let price = data
                    .get("c")
                    .and_then(|p| p.as_str())
                    ...
                    .ok_or_else(|| {
                        crate::raven_error!(parse_error, "Missing or invalid price".to_string())
                    })?;
                let wap = data
                    .get("w")
                    .and_then(|c| c.as_str())
        ...
        if let Some(stream) = value.get("stream").and_then(|s| s.as_str()) {
            if stream.ends_with("@trade") {
                ...
                let size = data
                    ...
                    .ok_or_else(|| {
                        crate::raven_error!(parse_error, "Missing or invalid quantity".to_string())
                    })?;
                ...
                let trade_id = data
                    ...
                    .map(|t| t.to_string())
                    .ok_or_else(|| {
                        crate::raven_error!(parse_error, "Missing trade ID".to_string())
                    })?;
        ...
        if let Some(stream) = value.get("stream").and_then(|s| s.as_str()) {
            if stream.ends_with("@aggTrade") {
                ...
                let trade_id = data
                    ...
                    .map(|a| a.to_string())
                    .ok_or_else(|| {
                        crate::raven_error!(parse_error, "Missing aggregate trade ID".to_string())
                    })?;
    ...
    fn parse_candle(&self, message: &str) -> RavenResult<Option<MarketDataMessage>> {
        ...
        if let Some(stream) = value.get("stream").and_then(|s| s.as_str()) {
            if stream.contains("@kline") {
                ...
                let kline = data.get("k").ok_or_else(|| {
                    crate::raven_error!(parse_error, "Missing kline data".to_string())
                })?;

                let symbol = kline.get("s").and_then(|s| s.as_str()).ok_or_else(|| {
                    crate::raven_error!(parse_error, "Missing symbol".to_string())
                })?;
                let open = kline
                    .get("o")
                    ...
                    .ok_or_else(|| {
                        crate::raven_error!(
                            parse_error,
                            "Missing or invalid open price".to_string()
                        )
                    })?;
                let high = kline
                    .get("h")
                    ...
                    .ok_or_else(|| {
                        crate::raven_error!(
                            parse_error,
                            "Missing or invalid high price".to_string()
                        )
                    })?;
                let low = kline
                    .get("l")
                    ...
                    .ok_or_else(|| {
                        crate::raven_error!(parse_error, "Missing or invalid low price".to_string())
                    })?;
                let close = kline
                    .get("c")
                    .and_then(|c| c.as_str())
                    ...
                    .ok_or_else(|| {
                        crate::raven_error!(
                            parse_error,
                            "Missing or invalid close price".to_string()
                        )
                    })?;
                let volume = kline
                    .get("v")
                    ...
                    .ok_or_else(|| {
                        crate::raven_error!(parse_error, "Missing or invalid volume".to_string())
                    })?;
                let timestamp = kline.get("t").and_then(|t| t.as_i64()).ok_or_else(|| {
                    crate::raven_error!(parse_error, "Missing or invalid timestamp".to_string())
                })?;
                let interval = kline.get("i").and_then(|i| i.as_str()).ok_or_else(|| {
                    crate::raven_error!(parse_error, "Missing interval".to_string())
                })?;
