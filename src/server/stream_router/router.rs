...
use crate::server::stream_router::{
    topic::TopicIndex, ClientSubscription, PersistedSubscription, SubscriptionDataType,
};
...
impl StreamRouter {
    ...
    pub fn subscribe(
        ...
        let exchanges_set: HashSet<String> = exchanges.into_iter().collect();
        ...
        let subscription = if let Some(mut existing) = self.subscriptions.get_mut(&client_id) {
            ...
            existing.exchanges.extend(exchanges_set.clone());
    ...
    pub fn unsubscribe(
        ...
        if let Some(mut subscription) = self.subscriptions.get_mut(client_id) {
            ...
            if !exchanges_set.is_empty() {
                for exchange in &exchanges_set {
                    subscription.exchanges.remove(exchange);
                }
            }
    ...
    pub fn distribute_message(
        ...
        for client_id in client_ids {
            if let Some(subscription) = self.subscriptions.get(&client_id) {
                ...
                if subscription.matches(symbol, &data_type, &message.exchange) {
    ...
    fn persist_subscription(&self, subscription: &ClientSubscription) {
        let persisted = PersistedSubscription {
            ...
            exchanges: subscription.exchanges.clone(),
