...
impl BinanceSpotOrderbook {
    ...
    pub async fn get_total_bid_volume(&self) -> Option<f64> {
        if let Some(data) = self.get_current_data().await {
            if let MarketData::OrderBook { bids, .. } = data.data {
                Some(bids.iter().map(|(_, volume)| volume).sum())
            } else {
                None
            }
        } else {
    ...
    pub async fn get_total_ask_volume(&self) -> Option<f64> {
        if let Some(data) = self.get_current_data().await {
            if let MarketData::OrderBook { asks, .. } = data.data {
                Some(asks.iter().map(|(_, volume)| volume).sum())
            } else {
                None
            }
        } else {
