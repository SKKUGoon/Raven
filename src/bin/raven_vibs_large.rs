...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // NOTE: We intentionally do NOT read vibs settings from TOML for this binary.
    ...
    let service_impl = vibs::new(upstreams, config, "vib_large".to_string());
    let raven = RavenService::new("RavenVibsLarge", service_impl.clone());
