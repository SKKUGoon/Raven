...
- **Deribit wildcard behavior**: for `--venue DERIBIT`, ravenctl intentionally starts `TICKER`, `TRADE`, and `PRICE_INDEX` streams with `symbol=*` so persistence can consume all-market Deribit feeds.
...
- Never ignore `start_collection` / `stop_collection` RPC results; always log failures so orchestration issues are visible.
