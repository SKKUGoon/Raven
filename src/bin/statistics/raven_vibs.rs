...
use raven::features::vibs;
use raven::service::RavenService;
use std::collections::HashMap;

#[derive(Parser, Debug)]
#[command(name = "raven_vibs")]
#[command(about = "Volume imbalance bars (VIBS) aggregator", long_about = None)]
struct Cli {
    /// Candle interval label (e.g. vib_small)
    #[arg(long, default_value = "vib")]
    interval: String,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let port = cli.port.unwrap_or(settings.server.port_vibs_small);
    ...
    let service_impl = vibs::new(upstreams, config, interval.clone());
    let service_name = format!("RavenVibs_{interval}");
